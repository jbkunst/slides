---
title: "{8:30}§2v|0i|2s|3u|2a|1l|0[-]i|1z|2a|1c|3[++]i|2o|1n|2{8abril}[26°c[10°c]"
...
O podemos concentrarnos en el grueso antes que los detalles...
...
pl +
  ...
  scale_fill_pres_d(breaks = c(1970, 1980, 1990, 2000, 2010), name = NULL) 
```
...
O simplemente contar una historia más clara/sencilla, pero no menos importante.

```{r}
ggplot(legos, aes(factor(year2))) +
  geom_bar(aes(fill = factor(year2)), width = 0.5, alpha = 0.85) +
  scale_fill_pres_d(guide = "none") +
...
```{r}
ggplot(dplazamaipu) +
  geom_col(aes(subidas_laboral_promedio, factor(format(mediahora, "%R")), fill = paraderosubida), size = 1.5) +
  scale_y_discrete(breaks = c("18:00", "12:00", "06:00" ), name = "mediahora") +
  scale_x_continuous(labels = comma) +
  scale_fill_pres_d(name = NULL)
```
...
Quizás mucha **tinta**? (_Data-Ink Ratio_).

```{r}
p0 <- ggplot(dplazamaipu) +
  geom_col(aes(mediahora, subidas_laboral_promedio, fill = paraderosubida), size = 1.5) +
...
Hay ciertas convenciones que no son malas seguir.
...
p1 <- ggplot(d1) +
  ...
  scale_color_pres_d(name = NULL, direction = -1)
...
ggmap <- OpenStreetMap::autoplot.OpenStreetMap(map.latlon) +
...
list(p0, p1, p2, pcors, pnet, pumapkm, pkm, pdend, pphylo, pfin, pmetro, ggmap) %>%
  map(function(p){
...
# <span style="background-color:white;margin:5px">Hay muchos mas!</span>
